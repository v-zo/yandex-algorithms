...
в коментариях в коде функции getSide. В зависимости от варианта вызывается бинарный поиск в "левой"
или "правой" половинке, а далеее алгоритм повторяет обычный бинарный поиск.
Хотя этот способ должен работать быстрее первого подхода, сложность всеравно та же - O(log n)
